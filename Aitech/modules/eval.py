...
async def executor(client: Rzayev, message: Message):
...
@Rzayev.on_cb("runtime")
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@Rzayev.on_cb("forceclose")
...
async def shellrunner(client: Rzayev, message: Message):
    if len(message.command) < 2:
